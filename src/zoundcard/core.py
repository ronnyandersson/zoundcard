...
class _Device(abc.ABC):
    ...
    def play(self, x, **kwargs):
        ...
        assert isinstance(x, zignal.Audio)
...
class Stub(_Device):
    ...
    def play(self, x, **kwargs):
        ...
        self._logger.warning("*** Stub play")

    def rec(self, duration=None, channels=1, fs=96000, **kwargs):
        _Device.rec(self, duration=duration, channels=channels, fs=fs, **kwargs)
        self._logger.warning("*** Stub rec")

        # fake a signal with white noise
        n = zignal.Noise(channels=channels, fs=fs, duration=duration, gaindb=-60)
        n.convert_to_float(targetbits=32)
        y = zignal.Audio(fs=fs, initialdata=n.samples)
        return y

    def play_rec(self, x, **kwargs):
        _Device.play_rec(self, x, **kwargs)
        self._logger.warning("*** Stub play_rec")

        # fake a signal with white noise
        n = zignal.Noise(channels=x.ch, fs=x.fs, nofsamples=x.nofsamples, gaindb=-60)
        n.convert_to_float(targetbits=32)
        y = zignal.Audio(fs=x.fs, initialdata=n.samples)
        return y
...
class PA(_Device):
    ...
    def __init__(self, device_out='default', device_in='default'):
        ...
        card is connected. The available sound cards can be found by calling
        list_devices()
        """
    ...
    def _validate(self, frames_per_buffer):
        assert hasattr(self, "pa"), \
            "Call open() or use the 'with' statement before using play(), rec() " + \
            "or play_rec()"
    ...
    def play(self, x, frames_per_buffer=1024):
        ...
        pad = zignal.Audio(
            channels=x.ch, fs=x.fs, nofsamples=missing_frames, dtype=x.samples.dtype)
        ...
        cpy = zignal.Audio(fs=x.fs, initialdata=x.samples)
        cpy.concat(pad)
        ...
        try:
            ...
            self._logger.debug("duration       : %.3f"  % (
                counter*frames_per_buffer/x.fs))
    ...
    def play_rec(self, x, frames_per_buffer=1024):
        ...
        rec = zignal.Audio(
            channels=cpy.ch, fs=cpy.fs, nofsamples=len(cpy), dtype=cpy.samples.dtype)
        ...
        try:
            ...
            try:
                while True:
                    ...
                    # because we use an iterator chunk_in is a sliding window in the
                    # rec variable
        ...
        self._logger.debug(
            "play_rec: trimmed %i samples from the start" % missing_frames)
        self._check_if_clipped(rec)
        self._logger.info("play_rec: done")

        return rec

    def rec(self, duration=None, channels=1, fs=96000, frames_per_buffer=1024,
            dtype=np.float32):
        ...
        rec = zignal.Audio(channels=channels, fs=fs, nofsamples=nofsamples, dtype=dtype)
        ...
        try:
            ...
            try:
                while True:
                    ...
                    raw_1d      = np.fromstring(stream.read(frames_per_buffer),
                                                dtype=rec.samples.dtype)
                    # because we use an iterator chunk_in is a sliding window
                    # in the rec variable
                    chunk_in[:] = raw_1d.reshape((frames_per_buffer, rec.ch))
            ...
            self._logger.debug("duration        : %.3f" % (
                counter*frames_per_buffer/rec.fs))
    ...
    def _check_if_clipped(self, rec):
        ...
        self._logger.debug(
            "maximum possible positive value: %i" % max_possible_positive_value)

        for i, peaks in enumerate(zip(rec.peak()[0], rec.peak()[1])):
            peak_val, peak_pos = peaks
            # abs(-32768) overflows in signed 16 bit, use long(...) in py2 to
            # get a bigger data type
            if abs(int(peak_val)) >= max_possible_positive_value:
                clipped = True
                clip_position = peak_pos/rec.fs
                self._logger.warning(
                    "channel %02i clipped at %.3f" % (i+1, clip_position))
